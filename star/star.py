...
class Star:
    ...
    async def setup_starboard(self, ctx, channel: discord.Channel=None, emoji="⭐", role:discord.Role=None, minimum=1):
        ...
        if minimum < 1:
            await self.bot.send_message(ctx.message.channel, "The minimum emoji must be 1 or greater!")
            return
    ...
    @starboard.command(pass_context=True, name="minimum", aliases="min")
    async def set_minimum(self, ctx, minimum=None):
        """Set the minimum number of emojis required for the starboard"""
        server = ctx.message.server
        if server.id not in self.settings:
            await self.bot.send_message(ctx.message.channel, 
                                        "I am not setup for the starboard on this server!\
        ...
        print(minimum)
        if minimum is None:
            await self.bot.send_message(ctx.message.channel, "Starboard minimum emojis set to {}.".format(self.settings[server.id]["minimum"]))
            return
        min_int = int(minimum)
        if min_int < 1:
            await self.bot.send_message(ctx.message.channel, "The minimum must be 1 or larger!")
            return
        print("???")
        self.settings[server.id]["minimum"] = min_int
        dataIO.save_json("data/star/settings.json", self.settings)
        await self.bot.send_message(ctx.message.channel, "Starboard minimum emojis set to {}.".format(minimum))
    ...
    async def get_posted_message(self, server, message, reaction):
        msg_list = self.settings[server.id]["messages"]
        for past_message in msg_list:
            if message.id == past_message["original_message"]:
                msg = past_message
        msg_list.remove(msg)
        if reaction.count > msg["count"]:
            msg["count"] = reaction.count
        msg_list.append(msg)
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if react in str(reaction.emoji):
            if emoji_count >= minimum:
                ...
                past_message_list.append({"original_message":msg.id, "new_message":post_msg.id,"count":reaction.count})
                dataIO.save_json("data/star/settings.json", self.settings)
            else:
                return
        else:
